...
evidence:
  ...
  - fixed upstream and live re-verified 2026-07-09: the same generic Mexico/MXN on-ramp query returns Etherfuse with coverage countries ["Mexico"], currencies ["USD","MXN"], and repos including etherfuse/ramp-api-example; upstream issue #7 is closed with maintainer reproduction notes
---
...
Live re-check 2026-07-09:

The same generic query now returns 17 records and includes Etherfuse with
`coverage:{countries:["Mexico"], currencies:["USD","MXN"], seps:[], asOf:"2026-07-07"}`
and repos including `etherfuse/ramp-api-example`. The named control query
`GET /api/projects/search?q=Etherfuse%20Stablebonds%20Mexico%20CETES` also
continues to return Etherfuse. Upstream issue
`Stellar-Light/stellar-scout#7` is closed, with maintainer comments confirming
structured coverage retrieval and the follow-up `ramps` filter correction.
